...
void S9xStartScreenRefresh ()
{
	...
	{
		GFX.PreviousFrameBrightness = PPU.Brightness;
		for (int i = 0; i < 16; i ++)
			GFX.PaletteFrame[i] ++;
...
inline void SelectTileRenderer (bool8 normal)
{
	if (Settings.SixteenBit)
	{
		//printf ("Select 16-bit renderers\n");
		if (normal || !Settings.Transparency)
		{
			DrawTilePtr = DrawTile16;
			DrawClippedTilePtr = DrawClippedTile16;
			DrawLargePixelPtr = DrawLargePixel16;
...
inline void SelectTileOBJRenderer (bool8 normal)
{
	if (Settings.SixteenBit)
	{
		//printf ("Select 16-bit renderers\n");
		if (normal || !Settings.Transparency)
		{
			DrawTilePtr = DrawTileOBJ16;
			DrawClippedTilePtr = DrawClippedTileOBJ16;
			DrawLargePixelPtr = DrawLargePixel16;
		}
		else
		{
...
inline void SelectTileBGRenderer (bool8 normal)
{
	if (Settings.SixteenBit)
	{
		//printf ("Select 16-bit renderers\n");
		if (normal || !Settings.Transparency)
		{
			DrawTilePtr = DrawTileBG16;
			DrawClippedTilePtr = DrawClippedTileBG16;
			DrawLargePixelPtr = DrawLargePixel16;
		}
		else
		{
...
if(Settings.BGLayering) {
			...
			{
				SelectTileOBJRenderer (!GFX.Pseudo && PPU.OBJ [S].Palette < 4);
			}
...
bool hasPriority1Tile[4][512]; 

void DrawBackgroundPriority1 (uint32 BGMode, uint32 bg)
{
    GFX.PixSize = 1;
...
void DrawBackground (uint32 BGMode, uint32 bg, uint8 Z1, uint8 Z2, int priority)
{
    GFX.PixSize = 1;
	...
	//if (priority == 0)
	//{
	//	for (uint32 Y = GFX.StartY; Y <= GFX.EndY; Y += 1)
	//		hasPriority1Tile[bg][Y] = false;	
	//}
	// ---
	
    for (uint32 Y = GFX.StartY; Y <= GFX.EndY; Y += Lines)
    {
		//if (priority == 1 && !hasPriority1Tile[Y])
		//	continue;	
...
void DrawBackdrop()
{
	...
	else*/
	{
		...
		{
			//printf ("Backdrop color %6x\n", back);
			uint32 backback = back | (back << 16);
			for (uint32 y = starty; y <= endy; y++)
			{
				...
				while (p < e)
				{
					...
					*((uint32 *)(p + 6)) = backback;
					*((uint32 *)(p + 8)) = backback;
					*((uint32 *)(p + 10)) = backback;
					*((uint32 *)(p + 12)) = backback;
					*((uint32 *)(p + 14)) = backback;
					//d++;
					p += 16;
				}
			}
...
{
	if (oldBGMode != PPU.BGMode)
	{
		//printf ("New BG Mode: %d\n", PPU.BGMode);
		oldBGMode = PPU.BGMode;	
	}
	...
	if (PPU.BGMode <= 6)
	{
		switch (PPU.BGMode)
		{
			...
			case 1:
				...
				DRAW_OBJS(1);
				DRAW_BG(1, D + 9, D + 13, 0);
				DRAW_BG(0, D + 10, D + 14, 0);
				DRAW_OBJS(2);
				DRAW_BG(1, D + 9, D + 13, 1);
				DRAW_BG(0, D + 10, D + 14, 1);
				DRAW_OBJS(3);
				if (PPU.BG3Priority)
				{
					DRAW_BG(2, D + 3, D + 17, 1);
				}
				break;
			case 2: 
				DrawBackdrop();
				DRAW_OBJS(0);
				DRAW_BG (1, D + 5, D + 13, 0);
				
				DRAW_OBJS(1);
				DRAW_BG (0, D + 5, D + 13, 0);
				
				DRAW_OBJS(2);
				DRAW_BG (1, D + 5, D + 13, 1);
				
				DRAW_OBJS(3);
				DRAW_BG (0, D + 5, D + 13, 1);
    ...
    else
    {
		if (Settings.SixteenBit)
		{
			...
			*/
			...
			uint32 *p = (uint32 *) (GFX.Screen + starty * GFX.Pitch2);
			uint32 *q = (uint32 *) (GFX.Screen + (endy + 1) * GFX.Pitch2);
			
			GX_MemoryFill((u32 *)p, back, (u32 *)q,  GX_FILL_TRIGGER | GX_FILL_16BIT_DEPTH, NULL, 0, NULL, 0);
		}
		else
		{
			for (uint32 y = starty; y <= endy; y++)
