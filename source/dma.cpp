...
// Define this to convert S9xGet/SetByte/Word into faster versions
// that do not do any check for CPU.InDMA
//
#define _IN_DMA_CPP_
...
uint32 S9xComputeDMABasePointer(uint8 bank, uint16 address, uint8 **base, uint16 *p)
{
	// Checks if the source address points to SRAM,
	// if so we compute the 'base'' and 'p'' a little differently,
	// since SRAM is not entirely linear.
	//
	uint32 finalAddress = (bank << 16) + address;
	uint8 *GetAddress = Memory.Map [(finalAddress >> MEMMAP_SHIFT) & MEMMAP_MASK];
	
	*base = GetBasePointer (finalAddress);
	*p = finalAddress & 0xffff;
	
	// Bug fix: Here we recompute the 'base' and 'p' for the SRAM
	// locations. This fixes some of the SMW hacks and makes 
	// texts readable. But the problem with DMA-ing from source 
	// address across multiple banks still exists. 
	//
	if ((uint32) GetAddress == CMemory::MAP_LOROM_SRAM)
	{
		*base = Memory.SRAM + ((((finalAddress&0xFF0000)>>1)|(finalAddress&0x7FFF)) & Memory.SRAMMask);
		*p = 0;
	}
	else if ((uint32) GetAddress == CMemory::MAP_HIROM_SRAM)
	{
		*base = Memory.SRAM + 
				((((finalAddress & 0x7fff) - 0x6000 +
				((finalAddress & 0xf0000) >> 3)) & Memory.SRAMMask));
		*p = 0;
	}
	...
	*/
    ...
    if (!d->TransferDirection)
    {
		...
		*/
		...
		if (memmap != CMemory::MAP_HIROM_SRAM && memmap != CMemory::MAP_LOROM_SRAM)
		{
		...
		{
			// Bug fix: Here we do special handling for 
			// source addresses that can wrap to the next bank.
			//
